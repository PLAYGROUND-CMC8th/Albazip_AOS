...
class FUnDoneAdapter(
    private val itemList: ArrayList<HUnDoneWorkListData>,
    context: Context,
    dialogView: View,
) : RecyclerView.Adapter<FUnDoneAdapter.UnDoneWorkHolder>(), PutTodayHomeTaskFragmentView {
    ...
    fun openContents(holder: UnDoneWorkHolder) {
        ...
        // 삭제 버튼 보이기(관리자만)
        if(itemList[holder.adapterPosition].posFlags == -2) {
