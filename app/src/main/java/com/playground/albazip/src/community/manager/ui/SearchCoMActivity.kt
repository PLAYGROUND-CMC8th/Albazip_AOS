...
import com.playground.albazip.src.community.common.network.SearchResult
...
class SearchCoMActivity:BaseActivity<ActivitySearchBinding>(ActivitySearchBinding::inflate),GetNoticeSearchFragmentView {
        ...
        binding.etvSearchArea.setOnKeyListener { _, keyCode, event ->
            ...
            if ((event.action== KeyEvent.ACTION_DOWN) && (keyCode == KeyEvent.KEYCODE_ENTER)) {
                ...
                if(binding.etvSearchArea.text.toString().replace(" ", "").equals("")){
                    ...
                    showLoadingDialog(this@SearchCoMActivity)
                }
    ...
    inner class CustomFragmentStateAdapter(fragmentActivity: FragmentActivity) :
        ...
        override fun createFragment(position: Int): Fragment {
            return when (position) {
                0 -> NoticeMSearchChildFragment(searchResponse)
