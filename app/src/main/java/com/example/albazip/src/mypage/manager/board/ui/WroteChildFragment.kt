...
class WroteChildFragment(val serverNoticeList:ArrayList<NoticeInfo>, val serverPostList:ArrayList<PostInfo>) : BaseFragment<ChildFragmentWroteBinding>(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        for(i in 0 until getPostList.size){
            boardList.add(BoardData(getPostList[i].writerName,getPostList[i].writerJob,getPostList[i].title,getPostList[i].content,getPostList[i].commentCount.toString(),getNoticeList[i].registerDate.substring(0,10).replace("-",".")+"."))
        }
